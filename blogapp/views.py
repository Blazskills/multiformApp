...
def home(request):
    ...
    # before you run elastic search or app , ensure you start elastic search server in the directory where elastic search is located
    if username:
    ...
    if ctype:
        activitystreams=activitystreams.filter("match", activitystream__target_ct_id=ctype)
        print(f'Activity stream ctype {activitystreams}')
