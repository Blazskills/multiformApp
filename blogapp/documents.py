...
from blogapp.models import NewComer, ActivityStream, ActivityStreamTarget
...
class ActivitystreamDocument(Document):
    ...
    activitystream = fields.NestedField(properties={
        'target_id': fields.TextField(),
        'target_ct_id': fields.TextField(),
      }, include_in_root=True)
  
    class Index:
        name = 'activitystream_multiform'
    settings = {
        'number_of_shards': 1,
        'number_of_replicas': 0,
    ...
    class Django:
        ...
        #  related_models = [User]
         related_models = [User]
         related_models = [ActivityStreamTarget]
    ...
    def get_instances_from_related(self, related_instance):
        with contextlib.suppress(Exception):
            """If related_models is set, define how to retrieve the Book instance(s) from the related model."""
            if isinstance(related_instance, ActivityStreamTarget):
                return related_instance.activitystream.all()
